<script lang="ts">
    ...
    $effect(() => {
        ...
        if (selectedId !== audioState.selectedDeviceId) {
            handleConnect();
        }
    });
...
<main
    class="min-h-screen bg-[#0f172a] text-slate-100 p-4 md:p-8 font-sans selection:bg-indigo-500/30"
>
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <header class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="p-2 bg-indigo-500 rounded-xl shadow-[0_0_20px_rgba(99,102,241,0.5)]"
                >
                    <Mic class="w-6 h-6 text-white" />
                </div>
                <h1
                    class="text-3xl font-bold tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent"
                >
                    X32 Recorder
                </h1>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                >
                    {#if audioState.isRecording}
                        <span
                            class="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,1)]"
                        ></span>
                        RECORDING
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
                <Card.Header>
                    ...
                    >
                        ...
                        {#if audioState.isPrimary}
                            <span
                                class="text-xs bg-amber-500/20 text-amber-400 px-2 py-1 rounded border border-amber-500/30 font-semibold"
                                >Primary</span
                            >
                        {:else}
                            <span
                                class="text-xs bg-slate-600/20 text-slate-400 px-2 py-1 rounded border border-slate-600/30 font-semibold"
                                >Secondary</span
                            >
                        {/if}
                    </Card.Title>
                ...
                <Card.Content class="space-y-4">
                    <div class="space-y-2">
                        <Label for="device" class="text-slate-500"
                        ...
                        <Select.Root
                            ...
                            disabled={audioState.isRecording ||
                                !audioState.isPrimary}
                        >
                            <Select.Trigger
                                class="bg-slate-950/50 border-slate-800 text-slate-200 focus:ring-indigo-500/50"
                            >
                                {audioState.devices.find(
                                    (d) => d.id === Number(selectedDeviceValue),
                                )?.name ?? "Select an interface..."}
                            </Select.Trigger>
                            <Select.Content
                                class="bg-slate-900 border-slate-800 text-slate-200"
                            >
                                {#each audioState.devices as device}
                                    ...
                                    >
                                        [{device.id}] {device.name} ({device.inputs}
                                        in)
                                    </Select.Item>
                                {/each}
                            </Select.Content>
            ...
            >
                ...
                <Card.Content class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <Label for="chL" class="text-slate-500"
                                >Left Channel</Label
                            >
                            <Input
                                type="number"
                        ...
                        <div class="space-y-2">
                            <Label for="chR" class="text-slate-500"
                                >Right Channel</Label
                            >
                            <Input
                                type="number"
                    ...
                    <div class="space-y-2">
                        <Label for="boost" class="text-slate-500"
                            >Gain Boost</Label
                        >
                        <Input
                            id="boost"
        ...
        <Card.Root
            class="bg-slate-900/40 border-slate-800 backdrop-blur-xl shadow-2xl"
        >
            <Card.Content class="pt-6 space-y-8">
                <!-- Main Controls -->
                <div
                    class="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-800/50"
                >
                    {#if audioState.isPrimary}
                        <div class="flex gap-4">
                            ...
                            >
                                <Play
                                    class="mr-3 w-6 h-6 fill-current pointer-events-none"
                                />
                                Start Recording
                            </Button>
                            <Button
                    ...
                    <div class="flex flex-col items-center md:items-end gap-2">
                        ...
                        >
                            <div class="flex items-center gap-2">
                                ...
                                <Label
                                    for="monitor"
                                    class="text-sm cursor-pointer select-none text-slate-300"
                                >
                        ...
                        <div
                            class="flex flex-col items-center md:items-end gap-2 text-right"
                        >
                            <div
                                class="flex items-center gap-2 text-[10px] text-slate-500 font-mono"
                            >
                                <span
                                    class="bg-slate-800 px-1.5 py-0.5 rounded text-indigo-400"
                                    >STORAGE</span
                                >
                            ...
                            >
                                <span
                                    class="bg-slate-800 px-1.5 py-0.5 rounded text-amber-500"
                                    >CLOUD</span
                                >
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="space-y-3">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center gap-3">
                                <span
                                    class="text-xs font-bold text-slate-500 w-4"
                                    >L</span
                                >
                                <div
                                    class="flex-1 h-3 bg-slate-950 rounded-full overflow-hidden border border-slate-800 shadow-inner p-0.5"
                                >
                                    <div
                                        class={cn(
                                            "h-full transition-all duration-75 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.3)]",
                                            audioVisuals.currentMeters.L > 95
                                                ? "bg-gradient-to-r from-emerald-500 via-yellow-400 to-red-500 shadow-red-500/20"
                                                : "bg-gradient-to-r from-emerald-600 to-emerald-400",
                                        )}
                                        style="width: {audioVisuals
                                            .currentMeters.L}%"
                                    ></div>
                                </div>
                                <span
                                    class="ml-3 text-xs text-slate-400 w-16 text-right"
                                    >{audioVisuals.currentDb.L <= -99
                                        ? "−∞"
                                        : `${audioVisuals.currentDb.L.toFixed(1)} dB`}</span
                                >
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                >
                                    <div
                                        class={cn(
                                            "h-full transition-all duration-75 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.3)]",
                                            audioVisuals.currentMeters.R > 95
                                                ? "bg-gradient-to-r from-emerald-500 via-yellow-400 to-red-500 shadow-red-500/20"
                                                : "bg-gradient-to-r from-emerald-600 to-emerald-400",
                                        )}
                                        style="width: {audioVisuals
                                ...
                                <span
                                    class="ml-3 text-xs text-slate-400 w-16 text-right"
                                    >{audioVisuals.currentDb.R <= -99
                                        ? "−∞"
                                        : `${audioVisuals.currentDb.R.toFixed(1)} dB`}</span
                                >
                            </div>
                        </div>
                    </div>
        ...
        <footer class="text-center py-8">
            <p
                class="text-xs text-slate-600 font-medium uppercase tracking-[0.2em]"
            >
                &copy; 2026 Behringer X32 Audio Engine &bull; Professional
                Edition
            </p>
        </footer>
    </div>
