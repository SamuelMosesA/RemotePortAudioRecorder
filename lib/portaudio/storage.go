			...
			case chunk, ok := <-recordChan:
				...
				if state.IsRecording {
					for i := 0; i < len(chunk); i += 2 {
						...
						binary.Write(state.FileL, binary.LittleEndian, iL)
						binary.Write(state.FileR, binary.LittleEndian, iR)
						binary.Write(state.FileStereo, binary.LittleEndian, iL)
						binary.Write(state.FileStereo, binary.LittleEndian, iR)
					}
